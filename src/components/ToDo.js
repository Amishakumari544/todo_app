...
export default (props) => {
	...
	const rlist = props.tasks.map((list, idx) =>
		<div key={idx}>
			...
			<Check className="check"  color="white" size={23} onClick={() => handleDone(list)} >check </Check>
			<X className="cross" color="white" size={23} onClick={() => handleChange(list, TASK_STATUSES.TO_DO)} />
		</div>
	);
